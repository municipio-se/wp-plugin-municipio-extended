  ...
  <{{ $componentElement }} class="{{ $class }}" aria-label="{{ $label }}" {!! $attribute !!}>
    ...
    <{{ $listType }} class="{{ $baseClass }}__list unlist">
      @foreach ($list as $item)
        <{{ $listItemType }} data-level="{{ $loop->depth }}"
          class="{{ $baseClass }}__item {{ $baseClass }}__item_{{ $loop->index }} {{ $baseClass }}__item_depth-{{ $loop->depth }}">

          @if (isset($item['icon']) && !empty($item['icon']))
            @icon([
                'icon' => $item['icon'],
                'classList' => [$item['icon'] == 'chevron_right' ? 'max-sm:rotate-180' : null]
            ])
            @endicon
          @endif
